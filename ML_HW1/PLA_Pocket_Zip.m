...
%generate data
% load zip.train %7291 * 257
% posIdx = find(zip(:,1)==1);
% posNum = length(posIdx);
% negIdx = find(zip(:,1)==5);
% negNum = length(negIdx);
% posSamples = zip(posIdx,2:end); %digit 1: N_positive * 256
% negSamples = zip(negIdx,2:end); %digit 5: N_negtive * 256
% % figure; imshow(reshape(negSamples(1,:),[16 16])',[]);
%
% X = double([posSamples; negSamples]);
% X = X';
% Y = double([ones(1,posNum) -ones(1,negNum)]);
[X, Y] = DataToXY('zip.train');
